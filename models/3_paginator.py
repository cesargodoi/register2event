...
class Paginator(UL):
    def __init__(
        ...
        **attributes
        ...
        if request.vars.pg_one:
            self.page = int(request.vars.pg_one)
        else:
            self.page = int(request.get_vars.get(self.page_var) or 1)
    ...
    def _url(self, page):
        ...
        return URL(args=request.args, vars=vars, anchor=self.anchor)
    ...
    def xml(self):
        ...
        if (
            self.records > self.paginate
            and pages == 1
            and pages * self.paginate != self.records
        ):
            pages = 2
        ...
        if pages > 1:
            ...
            def _get_page_el(page):
                if self.page == page:
                    return LI(A(page, _title=page, _href="#"), _class="active")
                else:
                    return LI(A(page, _title=page, _href=self._url(page)))
...
class PaginateSelector(DIV):
    def __init__(
        ...
        **attributes
        ...
        self.style = style
        self.paginate = int(
            request.get_vars.get(self.paginate_var, paginates[0])
        )
    ...
    def xml(self):
        if self.style == "text":
            ...
            inner = XML(
                ...
                + ", ".join(
                    [
                        _get_paginate_link(_paginate)
                        for _paginate in self.paginates
                    ]
                )
            )
            ...
            options = [
                OPTION(
                    self.messages.option % _paginate
                    if self.messages.option
                    else _paginate,
                    _value=_paginate,
                )
                for _paginate in self.paginates
...
class PaginateInfo(DIV):
    ...
    def xml(self):
        if self.records <= self.paginate:
            inner = XML(
                self.messages.display_without_span % dict(total=self.records)
            )
        else:
            inner = XML(
                self.messages.display_with_span
                % dict(
                    start=(self.page - 1) * self.paginate + 1,
