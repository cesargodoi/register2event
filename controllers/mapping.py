...
def new_mapping():
    query = (
        ...
        else (Events.id > 0) & (Events.center == auth.user.center)
    )
    ...
    new = SQLFORM.factory(
        Field("evenid", requires=IS_IN_SET(EVENTS_LIST), label=T("event")),
        buttons=[],
    )
    new["_id"] = "form"
    if new.process().accepted:
        evenid = (
            int(request.vars.evenid[0])
            if isinstance(request.vars.evenid, list)
            else int(request.vars.evenid)
        )
        Mapp.kill_mapping(evenid)
        Mapp.update_mapping(evenid)
        redirect(
            URL(
                "accommodations", "buildings_on_event", vars={"evenid": evenid}
            )
        )
