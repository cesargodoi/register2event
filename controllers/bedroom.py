...
def list_bedrooms_by_gender():
    ...
    bedrooms = db(
        (Bedroom.builid == builid) & (Bedroom.gender == gender)
    ).select(orderby=[~Bedroom.floor_room, Bedroom.bedroom])
    return dict(rows=bedrooms, gender=gender)
...
def new_bedroom():
    builid = (
        int(request.vars.builid[0])
        if isinstance(request.vars.builid, list)
        else int(request.vars.builid)
    )
    gender = (
        request.vars.gender[0]
        if isinstance(request.vars.gender, list)
        else request.vars.gender
    )
    new = SQLFORM(Bedroom, buttons=[])
    new["_id"] = "form"
...
def edit_bedroom():
    bedroomid = (
        int(request.vars.bedroomid[0])
        if isinstance(request.vars.bedroomid, list)
        else int(request.vars.bedroomid)
    )
    edit = SQLFORM(Bedroom, bedroomid, buttons=[])
    edit["_id"] = "form"
