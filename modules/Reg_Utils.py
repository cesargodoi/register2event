...
def updt_payforms(ptype, bflag, ctrl, gscred, value, payfid=None):
    ...
    previous = sum([float(pf.value) for pf in current.session.register.payforms])
    lacking = total - previous
    if (ptype in ("GSC", "CSH")) and value > lacking:
        value = lacking
    if ptype == "GSC":
        guest = [
            gs for gs in current.session.register.paying if gs.guesid == int(gscred)
        ][0]
        gsc_prev = sum(
            [
                float(pp.value)
                for pp in current.session.register.payforms
                if pp.ptype == "GSC" and pp.gscred == guest.guesid
            ]
        )
        ...
        current.session.register.payforms.append(
            Storage(ptype=ptype, gscred=int(gscred), value=value)
        )
    else:
...
def update_credit_and_log(guesid, evenid, value, oper):
    ...
    current.db.credit_log.insert(
        guesid=guesid, credit=value, oper=oper, credit_log=credit_log
    )
...
def adj_oldies():
    if current.session.register.old_regs:
        current.db(
            current.db.register.id.belongs(current.session.register.old_regs)
        ).delete()
    if current.session.register.old_payforms:
        current.db(
            current.db.payment_form.id.belongs(current.session.register.old_payforms)
        ).delete()
    if not current.session.register.delete_registers:
        if current.session.register.adjust_gscredit:
            for adj_gsc in current.session.register.adjust_gscredit:
                ...
                current.db.credit_log.insert(
                    guesid=adj_gsc.guesid, credit=adj_gsc.credit, credit_log=credit_log
                )
                current.db.commit()
...
def reconstruct(registers, payforms):
    for r in registers:
        ...
        gsc_in_payforms = [
            Storage(credit=pf.amount) for pf in payforms if pf.guesid == r.guesid
        ]
        ...
        for gsc_pf in gsc_in_payforms:
            ...
            current.session.register.adjust_gscredit.append(
                Storage(credit=gsc.credit, guesid=r.guesid)
            )
        payer = Storage(
            guesid=int(r.guesid),
            name=r.guesid.name,
